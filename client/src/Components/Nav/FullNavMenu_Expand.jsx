   ...
   const getUserOptions = (user) => {
     return generateUserOptions(user).map((option) => (
       <NavLink key={option.screen_name} className="userOption" to={`/${option.path}`}>
         <div className="rounded rounded-sm p-5 text-sm text-white hover:bg-neutral-100 hover:text-black lg:px-12 font-bold">
           {option.screen_name}
         </div>
       </NavLink>
     ));
    ...
    >
      ...
      <div className="flex pt-4" id="nav-options">
        ...
        <Popover className="relative">
          ...
          <Popover.Panel className="absolute right-0 z-10 rounded-md text-center bg-fuchsia-900 border border-black">
