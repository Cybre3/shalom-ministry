const Scripture = () => {
  ...
  return (
    <div className="lg:mt-10 h-screen w-screen bg-scriptureBlue bg-cover bg-center bg-no-repeat">
      <div className="container mx-auto flex h-full w-full flex-col items-center justify-center">
        <div className="mb-[-40px] flex h-1/2 flex-col italic text-white">
          <p className="w-full px-3 text-3xl font-thin lg:text-[5rem] lg:leading-[7rem]">
            {scriptureText}
            <span className="ml-3 space-x-3 text-xl lg:text-2xl">
              {scriptureReference.map((text) => text)}
            </span>
          </p>
          <p className="px-3 text-justify italic text-blue-800 lg:text-3xl">{scriptureSubText}</p>
        </div>
      </div>
    </div>
  );
